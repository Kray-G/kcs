...
/*
    $ cl /O2 utility\myacc.c
*/
#ifndef SMYY
#define SMYY "${yy_enough_long_space_to_replace}"
#endif
#ifndef LGYY
#define LGYY "${YY_enough_long_space_to_replace}"
#endif
...
/*
    Replacing string without allocation, buffer size must be enough.
*/
char *replace(char *str, const char *src, const char *dst)
{
    char *src_pos;
    if (!str || !src || !dst || !*src) {
        return str;
    }
    if ((src_pos = strstr(str, src)) == NULL) {
        return str;
    }

    const int src_len = strlen(src);
    const int dst_len = strlen(dst);
    const char *remain = src_pos + src_len;
    memmove(src_pos + dst_len, remain, strlen(remain) + 1);
    memcpy(src_pos, dst, dst_len);
    return str;
}

char *replace_all(char *str, const char *src, const char *dst)
{
    if (!str || !src || !dst || !*src) {
        return str;
    }
    ...
    const int src_len = strlen(src);
    const int dst_len = strlen(dst);
    while ((src_pos = strstr(src_pos, src)) != NULL) {
        const char *remain = src_pos + src_len;
        memmove(src_pos + dst_len, remain, strlen(remain) + 1);
        memcpy(src_pos, dst, dst_len);
    }
...
char *replace_yy(char *str)
{
    if (!str) {
        return str;
    }
    if (strstr(str, SMYY) == 0 && strstr(str, LGYY) == 0) {
        return str;
    }

    static char buf[2048] = {0};
    memcpy(buf, str, 2000);
    return replace_all(replace_all(buf, SMYY, smyy), LGYY, lgyy);
}
...
tblout()
{
	...
	fprintf(fout, replace_yy("short " SMYY "ini = %d;\n"), ini->id-1);
	fprintf(fout, replace_yy("short " SMYY "ntoks = %d;\n"), ntk);
	o = yalloc(nrl+nst+nsy, sizeof o[0]);
	for (n=0; n<nrl; n++)
		o[n] = slen(rs[n].rhs);
	aout(replace_yy(SMYY "r1"), o, nrl);
	for (n=0; n<nrl; n++)
		o[n] = rs[n].lhs-MaxTk;
	aout(replace_yy(SMYY "r2"), o, nrl);
	for (n=0; n<nst; n++)
		o[n] = as[n].def;
	aout(replace_yy(SMYY "adef"), o, nst);
	...
	aout(replace_yy(SMYY "gdef"), o, nsy-MaxTk);
	aout(replace_yy(SMYY "adsp"), adsp, nst);
	aout(replace_yy(SMYY "gdsp"), gdsp, nsy-MaxTk);
	for (n=0; n<actsz; n++)
		if (act[n]>=0)
			act[n]--;
	aout(replace_yy(SMYY "act"), act, actsz);
	aout(replace_yy(SMYY "chk"), chk, actsz);
	for (n=0; n<128; n++) {
		o[n] = 0;
		for (m=0; m<ntk; m++)
			if (is[m].name[0]=='\'')
	...
	aout(replace_yy(SMYY "trns"), o, m);
	if (fhdr) {
		fputs(replace_yy("int " SMYY "parse(void);\n"), fhdr);
		fputs(replace_yy("#ifndef " LGYY "STYPE\n"), fhdr);
		fputs(replace_yy("#define " LGYY "STYPE int\n"), fhdr);
		fputs("#endif\n", fhdr);
		fputs(replace_yy("extern " LGYY "STYPE " SMYY "lval;\n"), fhdr);
		fputs("#endif\n", fhdr);
	}
	free(o);
}
	...
	case '$':
		...
		if (c == '$') {
			...
			*ty = 0;
			...
			c = *p++;
			if (c == '$') {
				fprintf(fout, replace_yy(SMYY "val.%s"), ty);
			} else {
				if (!isdigit(c)) {
					lineno = i;
...
codeout()
{
	...
	for (p=code0; *p; p++)
		fputs(replace_yy(*p), fout);
	for (n=0; n<nrl; n++) {
		fprintf(fout, "\tcase %d:\n", n);
		r = &rs[n];
		if (r->actln > 0)
			fprintf(fout, "#line %d \"%s\"\n", r->actln, replace_sep(srca));
		actout(r);
		fputs("\t\tbreak;\n", fout);
	}
	for (p=code1; *p; p++)
