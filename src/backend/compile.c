...
/*
 * Check jump action for reasonable.
 */
#define JMP_CHECK(jmp0, jmp1) {\
    if (backjmp) {\
        emit(jmp1, OPT_IMM, addr(block->jump[1]->label));\
        reversed = 1;\
    } else {\
        emit(jmp0, OPT_IMM, addr(block->jump[0]->label));\
    }\
}\
...
static void emit(enum opcode opcode, enum instr_optype optype, ...)
{
    ...
    case OPT_IMM:
        ...
        if (instr.source.imm.d.addr.sym) {
            instr.source.imm.d.addr.label_name = str_init(sym_name(instr.source.imm.d.addr.sym));
        }
        break;
    case OPT_REG:
    ...
    case OPT_MEM:
        ...
        if (instr.source.mem.addr.sym) {
            instr.source.mem.addr.label_name = str_init(sym_name(instr.source.mem.addr.sym));
        }
        break;
    case OPT_REG_REG:
        instr.source.reg = va_arg(args, struct registr);
    ...
    case OPT_REG_MEM:
        ...
        if (instr.dest.mem.addr.sym) {
            instr.dest.mem.addr.label_name = str_init(sym_name(instr.dest.mem.addr.sym));
        }
        break;
    case OPT_MEM_REG:
        instr.source.mem = va_arg(args, struct memory);
...
static struct immediate constant(int64_t n, int w)
{
    struct immediate imm = {0};
    assert(w == 1 || w == 2 || w == 4 || w == 8);
    imm.type = IMM_INT;
...
static enum reg load_cast(struct var val, Type type)
{
    ...
    if (is_long_double(type)) {
        ...
        int to_bool = is_bool(type) && !is_bool(val.type);
        if (size_of(type) < 4) {
        ...
        if (is_real(val.type)) {
            ...
            load_int(val, r, w);
            if (to_bool) {
                emit(INSTR_TEST, OPT_REG_REG, reg(r, w), reg(r, w));
                emit(INSTR_SETNE, OPT_REG, reg(r, 1));
            }
        }
    }
...
static int enter(struct definition *def)
{
    int i, n, regs,
        ...
        vararg_offset = 0,  /* 16 bytes alignment of reg_offset. */
    ...
    if (res.eightbyte[0] == PC_MEMORY) {
        ...
        if (is_vararg(type)) {
            return_address_offset = -176 - 16 - reg_offset;
        }
    }
    ...
    #if defined(KCC_WINDOWS)
    if (is_main) {
        emit(INSTR_PUSH, OPT_REG, reg(DI, 8));
        emit(INSTR_PUSH, OPT_REG, reg(SI, 8));
    }
    #endif
    ...
    if (is_vararg(type)) {
        ...
        vararg.reg_save_area_offset = -vararg_offset;
...
static enum reg compile_call(struct var target, struct var ptr)
{
    ...
    if (ptr.kind == ADDRESS) {
        ...
        if (builtin) {
            #if defined(KCC_WINDOWS)
            /*
             *  builtin function can accept up to 4 arguments because
             *      Microsoft ABI will use only 4 registers for arguments.
             *  from:   rdi, rsi, rdx, rcx  ... (r8, r9)
             *  to:     rcx, rdx, r8, r9
             */
            /*
             *  Adjust Register position.
             */
            int reg_s[4] = {0, 0, 0, 0};
            int sse_s[4] = {0, 0, 0, 0};
            int exp_d[4] = {0, 1, 2, 3};
            int pushed_reg[4] = {-1, -1, -1, -1};
            uint8_t flbit = jit_get_builtin_flbit(sym_name(ptr.symbol));
            uint8_t args  = jit_get_builtin_args(sym_name(ptr.symbol));
            if (args > 4) {
                error("Do not use over 4 arguments in built-in functions(%s).", sym_name(ptr.symbol));
                exit(1);
            }
            int pushed = 0;
            if ((flbit & 0x01) == 0x01) {
            ...
            for (int i = args - 1; i >= 0; --i) {
                if (sse_s[i] >= 0 && sse_s[i] != exp_d[i]) {
                    emit(INSTR_MOVSD, OPT_REG_REG, reg(param_sse_reg[sse_s[i]], 8), reg(param_sse_reg[exp_d[i]], 8));
                }
                else if (reg_s[i] >= 0 && param_int_reg[reg_s[i]] != param_win_reg[exp_d[i]]) {
                    pushed_reg[pushed] = param_win_reg[exp_d[i]];
                    ++pushed;
                    emit(INSTR_PUSH, OPT_REG, reg(param_win_reg[exp_d[i]], 8));
                    emit(INSTR_MOV, OPT_REG_REG, reg(param_int_reg[reg_s[i]], 8), reg(param_win_reg[exp_d[i]], 8));
                }
            }
            int reserved_stack = 32 + 8 * (pushed & 0x01);
            emit(INSTR_SUB, OPT_IMM_REG, constant(reserved_stack, 8), reg(SP, 8));
            #endif
            emit(INSTR_MOV, OPT_IMM_REG, constant((uint64_t)builtin, 8), reg(R11, 8));
            emit(INSTR_CALL, OPT_REG, reg(R11, 8));
            #if defined(KCC_WINDOWS)
            emit(INSTR_ADD, OPT_IMM_REG, constant(reserved_stack, 8), reg(SP, 8));
            for (int i = 3; i >= 0; --i) {
                if (pushed_reg[i] >= 0) {
                    emit(INSTR_POP, OPT_REG, reg(pushed_reg[i], 8));
                }
            }
            #endif
        } else {
    ...
    struct var l)
{
    ...
    w = size_of(type);
    if (!is_array(type) && !is_standard_register_width(w) && w < 8) {
...
static void compile_block(struct block *block, Type type, int regs)
{
    ...
    if (block->body) {
        ...
        compile_block(block->body, type, regs);
    }
    ...
    if (block->body) {
    ...
    } else if (!block->jump[0] && !block->jump[1]) {
        ...
        #if defined(KCC_WINDOWS)
        if (is_main) {
            emit(INSTR_POP, OPT_REG, reg(SI, 8));
            emit(INSTR_POP, OPT_REG, reg(DI, 8));
        }
        #endif
        if (regs) {
            emit(INSTR_LEA, OPT_MEM_REG,
        ...
        int reversed = 0;
        int backjmp = block->jump[1]->color == BLACK && block->jump[0]->color == WHITE;
        if (is_comparison(block->expr)) {
            ...
            case INSTR_SETNE:
                if (is_real(block->expr.l.type)) {
                    ...
                    JMP_CHECK(INSTR_JE, INSTR_JNE);
                }
                break;
            case INSTR_SETG:
                JMP_CHECK(INSTR_JNG, INSTR_JG);
                break;
            case INSTR_SETNG:
                JMP_CHECK(INSTR_JG, INSTR_JNG);
                break;
            case INSTR_SETA:
                JMP_CHECK(INSTR_JNA, INSTR_JA);
                break;
            case INSTR_SETNA:
                JMP_CHECK(INSTR_JA, INSTR_JNA);
                break;
            case INSTR_SETGE:
                JMP_CHECK(INSTR_JNGE, INSTR_JGE);
                break;
            case INSTR_SETNGE:
                JMP_CHECK(INSTR_JGE, INSTR_JNGE);
                break;
            case INSTR_SETAE:
                JMP_CHECK(INSTR_JNAE, INSTR_JAE);
                break;
            case INSTR_SETNAE:
                JMP_CHECK(INSTR_JAE, INSTR_JNAE);
                break;
            }
        } else {
        ...
        if (block->jump[1]->color == BLACK) {
            if (!reversed) emit(INSTR_JMP, OPT_IMM, addr(block->jump[1]->label));
        } else {
...
static void compile_data_assign(struct var target, struct var val)
{
    ...
    if (val.kind == IMMEDIATE) {
        ...
        case T_FLOAT:
        case T_DOUBLE:
            if (type_of(target.type) == T_DOUBLE) {
            }
        case T_LDOUBLE:
...
static void compile_function(struct definition *def)
{
    ...
    #if defined(KCC_WINDOWS)
    is_main = !strcmp(sym_name(def->symbol), "main");
    #endif
...
INTERNAL void set_compile_target(FILE *stream, const char *file)
{
    ...
    case TARGET_IR_ASM:
        vm_init(NULL, NULL);
        flush_backend = vm_print_lir;
        finalize_backend = vm_finalize_lir;
        break;
    case TARGET_IR_RUN:
        vm_init(NULL, NULL);
        flush_backend = vm_run_lir;
        finalize_backend = vm_finalize_lir;
        break;
    case TARGET_IR_SAVE:
        vm_init(stream, file);
        flush_backend = vm_save_lir;
        finalize_backend = vm_finalize_lir;
        break;
    case TARGET_IR_DOT:
        dot_init(stream);
    ...
    case TARGET_x86_64_JIT_ASM:
        asm_init(stdout, NULL);
        jit_init();
        enter_context = jit_symbol;
        emit_instruction = jit_text;
        emit_data = jit_data;
        flush_backend = context.target == TARGET_x86_64_JIT ? jit_run : jit_print;
        finalize_backend = jit_finalize;
        break;
    ...
    case TARGET_x86_64_EXE:
        if (is_function(def->symbol->type)) {
            ...
            if (context.target != TARGET_x86_64_ASM &&
                context.target != TARGET_x86_64_JIT && context.target != TARGET_x86_64_JIT_ASM
            ) {
