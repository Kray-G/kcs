...
#define in_32bit_range(arg) ((arg) >= -2147483648LL && (arg) <= 2147483647LL)
 ...
 * fstp: 0x03
 */
static struct code x87_encode_transfer(uint32_t op, union operand m)
{
    struct code c = {0};
...
static struct code fild(uint32_t op, union operand m)
{
    struct code c = {0};
 ...
 * fdivrp: 0xF8
 */
static struct code x87_encode_arithmetic(uint32_t op, union operand r)
{
    struct code c = {0};
