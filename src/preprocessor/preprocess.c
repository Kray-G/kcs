...
static void preprocess_pragma(TokenArray *line)
{
    int i;
    struct token t;
    enum { PRAGMA_UNKNOWN, PRAGMA_IMPORT, PRAGMA_LIB, PRAGMA_DISWARN } id = PRAGMA_UNKNOWN;
    ...
    if (output_preprocessed) {
        ...
        for (i = 0; i < array_len(line); ++i) {
            ...
            if (t.token == IDENTIFIER) {
                if (!strcmp(str_raw(t.d.string), "import")) {
                    id = PRAGMA_IMPORT;
                } else if (!strcmp(str_raw(t.d.string), "lib")) {
                    id = PRAGMA_LIB;
                } else if (!strcmp(str_raw(t.d.string), "disable")) {
                    id = PRAGMA_DISWARN;
                }
            } else if (t.token == STRING || t.token == PREP_STRING) {
                switch (id)  {
                case PRAGMA_IMPORT:
                    add_import_module(str_raw(t.d.string));
                    break;
                case PRAGMA_LIB:
                    add_ref_module(str_raw(t.d.string));
                    break;
                case PRAGMA_DISWARN:
                    if (!strcmp(str_raw(t.d.string), "warning:all")) {
                        disable_warning(str_raw(t.d.string));
                    }
                    break;
                }
            }
