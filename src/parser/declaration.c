    ...
    size_t *length)
{
    ...
    default:
        if (base.type == T_ARRAY) {
            if (is_complete(base)) *type = type_copy_incomplete(base);
            else                   *type = base;
        }
        else {
            *type = base;
    ...
    if (t.token == IDENTIFIER) {
        ...
        if (peek().token == '{') {
            if (tag->value.constant.i) {
                error("Redefinition of enum '%s'.", str_raw(tag->name));
                exit(1);
            }
            enumerator_list();
    ...
    int *is_register)
{
    ...
    case B_DOUBLE:
        type.type = T_DOUBLE;
        // As long double in Microsoft Visual Studio is same as double, it is just used as double also with kcc.
            // if (modifier == M_LONG) {
            //     type.type = T_LDOUBLE;
            // }
        break;
    ...
    enum linkage linkage)
{
    ...
    if (!is_complete(sym->type) && is_array(sym->type) && size_of(sym->type)) {
        set_array_length(sym->type, type_array_len(sym->type));
    }
    ...
    if (symtype == SYM_TENTATIVE || (symtype == SYM_DECLARATION && linkage == LINK_EXTERN)) {
        /*
         * SYM_TENTATIVE would be set for global or static variable.
         * Also LINK_EXTERN means global variable.
         */
        if (sym->global_offset < 0) {
            sym->global_offset = get_global_variable_size();
            add_global_variable_size(size_of(sym->type));
        }
    }
    return parent;
}
    ...
    struct block *parent)
{
    ...
    if (peek().token == NORETURN) {
        next(); /* ignore _Noreturn. */
    }
