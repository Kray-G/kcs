...
static struct var imm_signed(Type type, int64_t n)
{
    union value val = {0};
...
INTERNAL struct var imm_unsigned(Type type, uint64_t n)
{
    union value val = {0};
    ...
    case T_SHORT:
        if (is_signed(to)) {
            val.i = is_float_below(val, type, SHRT_MIN) ? SHRT_MIN
                  ...
                  : cast_immediate(val, type, int16_t);
        } else {
            val.u = is_float_below(val, type, 0) ? 0
                  : is_float_above(val, type, USHRT_MAX) ? USHRT_MAX
                  : cast_immediate(val, type, uint16_t);
        }
        break;
    case T_INT:
        if (is_signed(to)) {
            val.i = is_float_below(val, type, INT_MIN) ? INT_MIN
                  ...
                  : cast_immediate(val, type, int32_t);
        } else {
            val.u = is_float_below(val, type, 0) ? 0
                  : is_float_above(val, type, UINT_MAX) ? UINT_MAX
                  : cast_immediate(val, type, uint32_t);
        }
        break;
    case T_LONG:
        if (is_signed(to)) {
            val.i = is_float_below(val, type, LONGLONG_MIN) ? LONGLONG_MIN
                  : is_float_above(val, type, LONGLONG_MAX) ? LONGLONG_MAX
                  : cast_immediate(val, type, int64_t);
        } else {
    case T_POINTER:
            val.u = is_float_below(val, type, 0) ? 0
                  : is_float_above(val, type, ULONGLONG_MAX) ? ULONGLONG_MAX
                  : cast_immediate(val, type, uint64_t);
        }
        break;
    default: assert(0);
    }
    ...
    struct expression expr)
{
    ...
    if (!size_of(target.type)) {
        ...
        if (size_of(expr.type) == size_of(target.type) + 1) {
            ...
            warning("Length of string literal exceeds target array size.");
            expr.type = target.type;
            expr.l.type = target.type;
        }
