...
INTERNAL const Type
    basic_type__void           = { .type = T_VOID },
    basic_type__bool           = { .type = T_BOOL },
    basic_type__char           = { .type = T_CHAR },
    basic_type__short          = { .type = T_SHORT },
    basic_type__int            = { .type = T_INT },
    basic_type__long           = { .type = T_LONG },
    basic_type__unsigned_char  = { .type = T_CHAR,  .is_unsigned = 1 },
    basic_type__unsigned_short = { .type = T_SHORT, .is_unsigned = 1 },
    basic_type__unsigned_int   = { .type = T_INT,   .is_unsigned = 1 },
    basic_type__unsigned_long  = { .type = T_LONG,  .is_unsigned = 1 },
    basic_type__float          = { .type = T_FLOAT },
    basic_type__double         = { .type = T_DOUBLE },
    basic_type__long_double    = { .type = T_LDOUBLE };
...
INTERNAL Type type_copy_incomplete(Type type)
{
    ...
    struct typetree *tb = get_typetree_handle(type.ref);
    struct typetree *tn = get_typetree_handle(tp.ref);
    *tn = *tb;
    tn->is_incomplete = 1;
    return tp;
}
