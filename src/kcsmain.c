...
{
    // Search in kcs include path first.
    add_include_search_path(make_path(get_exe_path(), "kcsrt/include"));
    add_include_search_path("/usr/local/lib/kcsrt/include");
...
DLLEXPORT int kcsmain(int argc, char *argv[])
{
    int i, ret;
    struct input_file file;
