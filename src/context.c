 ...
 * formatters: %s, %c, %d, %lu, %ld, %llu, %lld, %%.
 *
        ...
        if (c != '%') {
            ...
            case 'l': {
                ...
                case 'l': {
                    ...
                    default: assert(0);
                    case 'u':
                        n += fprintf(stream, "%llu", va_arg(ap, uint64_t));
                        break;
                    case 'd':
                        n += fprintf(stream, "%lld", va_arg(ap, int64_t));
                        break;
                    }
                    break;
                }
                case 'u':
                    n += fprintf(stream, "%lu", va_arg(ap, uint32_t));
                    break;
                case 'd':
                    n += fprintf(stream, "%ld", va_arg(ap, int32_t));
                    break;
                }
                break;
            }
...
{
    /*
     * Only the mode "all" is currently supported.
     */
    if (!strcmp(mode, "warning:all")) {
