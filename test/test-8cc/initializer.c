...
#define verify(expected, got, len)          fverify(__FILE__, __LINE__, expected, got, len)
#define verify_short(expected, got, len)    fverify_short(__FILE__, __LINE__, expected, got, len)

static void fverify(const char *f, int l, int *expected, int *got, int len) {
    for (int i = 0; i < len; i++) {
        // printf("[%d] = %d <=> %d\n", i, expected[i], got[i]);
        expect_fl(f, l, expected[i], got[i]);
    }
}

static void fverify_short(const char *f, int l, short *expected, short *got, int len) {
    for (int i = 0; i < len; i++)
        expect_fl(f, l, expected[i], got[i]);
}
